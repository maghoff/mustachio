...
Sequence.prototype.generateCode = function () {
	const code = [];
	for (const item of this.seq) {
		code.push(item.generateCode());
	}
	return code.join('\n');
};
...
Section.prototype.generateCode = function () {
	return `{
	const maybeResolved = resolve(context, ${JSON.stringify(this.path)});
	const value = (maybeResolved instanceof Promise) ? (yield maybeResolved) : maybeResolved;

	yield* renderSection(value, (function*(context){'use strict';${this.nested.generateCode()}}), context);
	}`;
};
...
NegativeSection.prototype.generateCode = function () {
	return `{
	const maybeResolved = resolve(context, ${JSON.stringify(this.path)});
	const value = (maybeResolved instanceof Promise) ? (yield maybeResolved) : maybeResolved;
	...
	do {
		...
		yield* (function*(context){${this.nested.generateCode()}})(context);
	} while(false);
	}`;
};
