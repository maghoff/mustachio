...
Interpolation.prototype.render = function* (context) {
	...
	if (isStream.readable(value)) {
		yield* consumeStream(value, chunk => {
			if (this.verbatim) return [chunk];
			else return escape(chunk);
		});
	} else {
		if (this.verbatim) yield stringify(value);
		else yield* escape(stringify(value));
	}
}
...
// 	}
// };
function* resolveYieldPromiseThing(iterable, nested, context) {
